...
import { searchGitHubRepos, type GitHubRepo } from "../services/github.js";
...
async function saveSnapshot(repoId: number, repo: GitHubRepo): Promise<void> {
  await pool.query(
...
async function getSnapshotStars(repoId: number, hours: number): Promise<number | null> {
...
function computeScore(repo: GitHubRepo, stars24h: number, stars7d: number): number {
...
async function updateRepoScores(repoId: number, repo: GitHubRepo): Promise<void> {
